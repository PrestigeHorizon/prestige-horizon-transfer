...
import {
  ...
  Eye, EyeOff, Lock, CheckCircle2, Send, Clock,
} from 'lucide-react';
...
const Profile = () => {
  ...
  const handlePhoneSave = async () => {
    ...
    try {
      ...
      /* Route optionnelle — si pas encore implémentée, afficher un toast info */
    ...
    <div className="min-h-screen bg-[#050505]" data-testid="profile-page">
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-[#A1A1AA] mt-1">Gérez vos informations et votre sécurité</p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            <Card className="bg-[#0F0F0F] border-white/10">
              <CardContent className="p-4 space-y-2">
                <Link to="/new-transfer" className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-colors">
                  ...
                  <span className="text-white text-sm font-medium">Nouveau transfert</span>
                </Link>
                <Link to="/transfers" className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-colors">
                  ...
                  <span className="text-white text-sm font-medium">Historique</span>
                </Link>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="bg-[#0F0F0F] border-white/10" data-testid="account-info">
              ...
              <CardContent>
                <InfoRow icon={User}   label="Nom complet"  value={user.full_name} />
                <InfoRow icon={Mail}   label="Email"        value={user.email} />
                <InfoRow icon={MapPin} label="Pays"         value={user.country} data-testid="profile-country" />
                ...
                <div className="flex items-center gap-3 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[#A1A1AA] text-xs uppercase tracking-wider">Téléphone</p>
                    {editPhone ? (
            ...
            <Card className="bg-[#0F0F0F] border-white/10" data-testid="security-section">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-white text-base">Sécurité</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                  ...
                  {pwSection && (
                    <div className="space-y-3 pt-1">
                      ...
                      <Input
                        ...
                        className={`bg-[#0F0F0F] border-white/10 focus:border-[#D4AF37] text-white text-sm ${
                          pwData.confirm && pwData.confirm !== pwData.new_password ? 'border-red-500/50' : ''
                        }`}
                      />
                      ...
                      >
                        {pwLoading ? 'Enregistrement…' : 'Enregistrer le nouveau mot de passe'}
                      </Button>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-[#1A1A1A] rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-white text-sm font-medium">Se déconnecter</p>
                      <p className="text-[#A1A1AA] text-xs">Quitter la session sur cet appareil</p>
                    </div>
                  </div>
                  <Button
