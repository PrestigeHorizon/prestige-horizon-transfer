...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all prestige.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*prestige\.sh$/)) {
        return true;
      }

      // Allow all prestigeagent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*prestigeagent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@prestige.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@prestige.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
